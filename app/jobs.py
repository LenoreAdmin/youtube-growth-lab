...
def acquire(session_factory, bucket=None):
    owner = uuid4().hex
    with session_factory.begin() as s:
        ...
        s.execute(insert(JobLease).values(name="youtube-sync").on_conflict_do_nothing(index_elements=["name"]))
        now = aware(s.scalar(select(func.now())))
        conditions = [JobLease.name == "youtube-sync",
        ...
        if updated.rowcount == 1:
            return owner, None
        row = s.get(JobLease, "youtube-sync")
        return None, "already_completed" if bucket and row.completed_bucket == bucket else "already_running"


def release(session_factory, owner, bucket=None):
    with session_factory.begin() as s:
        values = {"owner": None, "expires_at": None}
        if bucket is not None:
            values["completed_bucket"] = bucket
        s.execute(update(JobLease).where(JobLease.name == "youtube-sync", JobLease.owner == owner).values(**values))
