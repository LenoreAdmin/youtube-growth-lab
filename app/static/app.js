...
function growthCard(g){
 ...
 return `<div class="card"><p class="eyebrow">GROWTH ENGINE · V5</p><h3>${esc(STATE_LABELS[g.state]||g.state)} → ${esc(ACTION_LABELS[g.action]||g.action)}</h3><p class="muted">Stand ${esc(g.day)} · Revival: ${g.revival?.candidate?"Kandidat ("+(g.revival.signals||[]).map(esc).join(", ")+")":esc(g.revival?.reason||"nein")} · Live-Momentum V2: ${g.momentum?.score==null?"—":num(g.momentum.score,1)}</p>${table("Growth Opportunity",g.opportunity)}${table("Viewer Acquisition",g.viewer)}${table("Subscriber Opportunity",g.subscriber)}<h3>Aktionen und Auswertung</h3>${acts}</div>`;
}
